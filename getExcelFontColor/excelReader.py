...
class readexcel(object):
    """ Simple OS Independent Class for Extracting Data from Excel Files 
        ...
        Author:snowzjy          Email:farsoftwind@gmail.com         
        Versions of Excel supported: 2004, 2002, XP, 2000, 97, 95, 5, 4, 3
        xlrd version tested: 0.6.1         
    ...
    def getxfiter(self, sheetname, returnlist=False, returntupledate=False):
        """ Return an generator object which yields the lines of a worksheet;
        Default returns a dictionary, specifing returnlist=True causes lists
        to be returned.  Calling returntupledate=True causes dates to returned
        as tuples of (Year, Month, Day, Hour, Min, Second) instead of as a
        ...
        if sheetname not in self.__sheets__.keys():
            raise NameError, "%s is not present in %s" % (sheetname,\
                                                          self.__filename__)
        if returnlist:
            return __XF_iterlist__(self, sheetname, returntupledate)
        else:
            return __XF_iterdict__(self, sheetname, returntupledate) 
    def worksheets(self):
...
#----------------------------------------------------------------------------- 
def get_color(book, color, styleName) :
    #Trying default value
    if color == None:
        color = book.colour_map[0x7FFF]
    colorStyle=''
    if not color == None:
        colorStyle='%s(%d,%d,%d)' % (styleName,color[0],color[1],color[2])
    return colorStyle 
def __XF_iterlist__(excel, sheetname, tupledate):
    import xlrd
    """ Function Used To Create the Dictionary Iterator """
    sheet = excel.__book__.sheet_by_name(sheetname)
##    if not sheet.nrows:
##        return None # skip empty sheets
    ...
    for ridx in range(excel.__sheets__[sheetname]['rows']):
        types,values = sheet.row_types(ridx),sheet.row_values(ridx)
        rowvalue = excel.__formatrow__(types, values, tupledate)         
        bufxf = []
        row = sheet.row(ridx)
        ...
        for cidx, cell in enumerate(row):             
            key = (ridx,cidx)
            if key in skipped and skipped[key]:
                continue #skipping - merged columns
            formatIndex = sheet.cell_xf_index(ridx, cidx)
            format = excel.__book__.computed_xf_list[formatIndex]
            font = excel.__book__.font_list[format.font_index]
            color = excel.__book__.colour_map[font.colour_index]
            bgColor = excel.__book__.colour_map[format.background.pattern_colour_index]
            style = 'normal'
            if font.italic:
                style='italic'
            decor = 'none'
            ...
            if key in merged:
                spans = merged[key]
                rowspan='rowspan="%d"' % spans[0]
                colspan='colspan="%d"' % spans[1] 
            if not color == None and bgColor == None:
                print(u'col%d' % (cidx + 1)),
            #buf.append(u'style="font-weight:%(weight)d; font-style:%(style)s; %(colorStyle)s;  
#\%(bgColorStyle)s;\
            #               text-decoration:%(decor)s " %(rowspan)s %(colspan)s>%(value)s' %
            print(u'%(colorStyle)s;%(bgColorStyle)s' %
                {
                #"weight":font.weight,
                #"style":style,
                #"value":sheet.cell_value(ridx, cidx),
                "colorStyle":get_color(excel.__book__,color,'fc'),
                "bgColorStyle":get_color(excel.__book__,bgColor,'bc'),
                #"decor":decor,
                #"colspan":colspan,
                #"rowspan":rowspan
                }), 
            xfc =  (rowvalue[cidx],get_color(excel.__book__,color,'fc'),get_color(excel.__book__,bgColor,'bc'))
            bufxf.append(xfc) 
        yield bufxf 
def __XF_iterdict__(excel, sheetname, tupledate):
    import xlrd
    """ Function Used To Create the Dictionary Iterator """
    sheet = excel.__book__.sheet_by_name(sheetname)
##    if not sheet.nrows:
    ...
    for crange in sheet.merged_cells:
        ...
        for rx in xrange(rlo, rhi):
            for cx in xrange(clo, chi):
                skipped[(rx,cx)] = True
        skipped[(rlo,clo)] = False
    for ridx in range(excel.__sheets__[sheetname]['firstrow'],\
                     excel.__sheets__[sheetname]['rows']):
        types,values = sheet.row_types(ridx),sheet.row_values(ridx)
        rowvalue = excel.__formatrow__(types, values, tupledate)         
        bufxf = []
        ...
        for cidx, cell in enumerate(row):
            ...
            xfc = (rowvalue[cidx],u'%(colorStyle)s'%{"colorStyle":get_color(excel.__book__,color,'fc')},\
                         u'%(bgColorStyle)s'%{"bgColorStyle":get_color(excel.__book__,bgColor,'bc')})
            bufxf.append(xfc) 
        yield dict(zip(excel.__sheets__[sheetname]['variables'],bufxf))